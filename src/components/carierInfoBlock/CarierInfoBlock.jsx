...
const CarierInfoBlock = ({ variant }) => {
  ...
  const title =
    variant == "practicum" ? t("carier.practicumtitle") : t("carier.teamTitle");
  const description =
    variant == "practicum"
      ? t("carier.practicumDescription")
      : t("carier.teamDescription");
  const data = variant == "practicum" ? practicumStepsList : teamStepsList;

  const showLine = useMemo(() => {
    if (variant == "team") {
      return <div className={scss.carier__line}></div>;
    }
  }, [variant]);
  ...
  return (
    <>
      <Navigation title={t("navigation.Career_description")} />
      <div className={scss.carier__wrapper}>
        <div className={scss.text__content}>
          ...
          <Button
            ...
            withArrow={true}
            title={t("carier.btnText")}
          />
        </div>
        <div className={scss.steps__container}>{renderCards}</div>
        {showLine}
      </div>
    </>
  );
